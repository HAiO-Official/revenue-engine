...
3. **Transparent Distribution (On-Chain Programs):**
   ...
   * It reads the `$HAiO` balance in the `Revenue Safe` and the distribution ratios (β% for Stakers, γ% for DAO, δ% for Devs) stored in the `EngineState Account`.
...
│   ├── revenue_engine/   # Core distribution logic & state
│   ├── staking_program/  # NFT staking & reward claim logic
│   └── mock_swap_program/ # Demo swap functionality (USDC<>HAiO, USDC<>ATH)
├── app/                  # Off-Chain Agent (Worker) implementation
│   └── src/agent.ts      # Main worker logic
├── scripts/              # Initialization and helper scripts
│   └── init.ts           # Sets up accounts, mints tokens, initializes programs
├── tests/                # End-to-end tests
│   └── haio-seoulana.ts
├── keypairs/             # Keypairs for Devnet testing (Mints, Wallets)
├── target/               # Compiled artifacts (IDLs, types) - Gitignored usually
├── frontend/             # React Frontend for Demo (Optional)
...
- Manages the global EngineState Account, which stores critical parameters like distribution ratios (β, γ, δ), total staked amount, and the cumulative reward rate.
...
- **Purpose**: Stores the global configuration and real-time economic state for the RevenueEngine. This includes distribution ratios (β, γ, δ), the address of the Revenue Safe and destination PDAs, the current total_staked_amount, and the crucial reward_per_token_cumulative for lazy reward calculations.
...
- **Mint**: $HAiO.
- **Authority (to transfer from)**: EngineState PDA. This is crucial – only the RevenueEngine program, acting through its PDA, can move funds out of the safe during distribution.

#### Reward Pool PDA Account (SPL Token Account):
- **Purpose**: Holds the accumulated $HAiO allocated for NFT staker rewards (β%) after distribution from the Revenue Safe.
- **Mint**: $HAiO.
...
- **Purpose**: Holds the accumulated $HAiO allocated for DAO operations (γ%) after distribution.
- **Mint**: $HAiO.
...
- **Purpose**: Holds the accumulated $HAiO allocated for developer incentives/rewards (δ%) after distribution.
- **Mint**: $HAiO.
